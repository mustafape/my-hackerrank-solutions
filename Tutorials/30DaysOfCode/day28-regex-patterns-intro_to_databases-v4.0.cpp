...
// Dump Strategy Interface
class DumpStrategy {
    public:
		enum DumpCriteria {dc_NAME_ONLY, dc_EMAIL_ONLY, dc_ALL, dc_UNDEFINED};	//	Dump criteria

		virtual void execute(vector<PersonRecord>& records, bool isFilterActive) = 0; // Pure virtual function
		virtual ~DumpStrategy() {} // Virtual destructor
};

// Concrete Dump Strategy for dumping by name only
class DumpByNameOnly : public DumpStrategy {
    public:
		void execute(vector<PersonRecord>& records, bool isFilterActive) override {
            for (int i=0; i<records.size(); ++i) {
                if(isFilterActive && records[i].hidden_)
                    continue;
                else
                    cout << records[i].first_name_ << endl;
            }
        }
};

// Concrete Dump Strategy for dumping by name only
class DumpByeMailOnly : public DumpStrategy {
    public:
		void execute(vector<PersonRecord>& records, bool isFilterActive) override {
            for (int i=0; i<records.size(); ++i) {
                if(isFilterActive && records[i].hidden_)
                    ...
                    cout << records[i].email_id_ << endl;
            }
        }
};

// Concrete Dump Strategy for dumping by name only
class DumpByAll : public DumpStrategy {
    public:
		void execute(vector<PersonRecord>& records, bool isFilterActive) override {
            for (int i=0; i<records.size(); ++i) {
                if(isFilterActive && records[i].hidden_)
                    ...
                    cout << records[i].first_name_ << " " << records[i].email_id_ << endl;
            }
        }
};
...
class EmailsDBTable
{
	...
	private:
        ...
        unique_ptr<DumpStrategy>	dumpStrategy_;	// Dump Strategy
    ...
    public:
        ...
        void setDumpStrategy(unique_ptr<DumpStrategy> strategy) {
            dumpStrategy_ = move(strategy);
        }

        void executeDump() {
            if (dumpStrategy_) {
                dumpStrategy_->execute(record_collection_, isFilterActive());
            } else {
                cout << "No dump strategy set" << endl;
            }
        }
...
public:
	void run()
	{
        ...
        myDB.setDumpStrategy(make_unique<DumpByNameOnly>());
		myDB.executeDump();
	}
};
